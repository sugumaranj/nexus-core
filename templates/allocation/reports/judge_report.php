...
/**
 * -------------------------------------------------------------------------
 * NexusCore EMS — Judge Assignment Report
 * Location    : templates/allocation/reports/judge_report.php
...
foreach ($assignments as $a) {
    ...
    $byEvent[$eid]['event']     = $a;
    $byEvent[$eid]['judges'][]  = $a;
}

$totalJudges = count($assignments);
...
<head>
    <meta charset="UTF-8">
    <title>Judge Assignment Report — <?= $symTitle ?></title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .report-header { text-align: center; border-bottom: 2px solid #059669; padding-bottom: 12px; margin-bottom: 16px; }
        .college-name { font-size: 13.5pt; font-weight: 700; color: #064e3b; margin-bottom: 2px; }
        .report-title  { font-size: 11.5pt; font-weight: 700; color: #059669; margin: 6px 0 2px; }
        ...
        .section-title { font-size: 9.5pt; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: #059669; margin: 14px 0 6px; border-bottom: 1px solid #e2e8f0; padding-bottom: 4px; }
        table { width: 100%; border-collapse: collapse; font-size: 9.5pt; margin-bottom: 14px; }
        th { background: #059669; color: white; padding: 7px 10px; font-weight: 600; font-size: 8.5pt; text-transform: uppercase; letter-spacing: .05em; text-align: left; }
        td { padding: 7px 10px; border-bottom: 1px solid #e2e8f0; vertical-align: top; }
        tr:nth-child(even) td { background: #f0fdf4; }
        .event-row td { background: #dcfce7; font-weight: 700; color: #059669; font-size: 9pt; }
        ...
        .total-row td { font-weight: 700; background: #dcfce7 !important; color: #059669; }
        @media print {
...
<div class="no-print" style="text-align:right; padding:8px 12px; background:#f8f9fa; border-bottom:1px solid #e2e8f0; margin-bottom:16px;">
    <button onclick="window.print()" style="background:#059669; color:white; border:none; padding:6px 18px; border-radius:6px; cursor:pointer; font-size:10pt;">
        🖨 Print / Save PDF
    </button>
...
<div class="report-header">
    ...
    <div class="report-title">JUDGE ASSIGNMENT REPORT</div>
    <div class="report-sub"><?= $symTitle ?> &bull; Academic Year: <?= $symYear ?></div>
</div>

<div class="meta-row">
    <div><strong>Report ID:</strong> JDG-<?= date('Ymd') ?>-<?= str_pad((string)($symposium['symposium_id'] ?? 0), 4, '0', STR_PAD_LEFT) ?></div>
    <div><strong>Total Events:</strong> <?= $totalEvents ?> &nbsp;|&nbsp; <strong>Total Judges:</strong> <?= $totalJudges ?></div>
...
<table>
    <thead>
        <tr>
            ...
            <th style="width:20%">Judge Name</th>
            <th style="width:13%">Department</th>
        </tr>
    </thead>
    <tbody>
        <?php if (empty($assignments)): ?>
        <tr><td colspan="8" style="text-align:center; color:#9ca3af; padding:20px;">No judge assignments found for this symposium.</td></tr>
        <?php else: ?>
        <?php $n = 1; foreach ($byEvent as $eid => $row):
            ...
            foreach ($judges as $idx => $judge):
        ?>
        <tr>
            <td><?= $n++ ?></td>
            <?php if ($idx === 0): ?>
            <td rowspan="<?= count($judges) ?>"><?= htmlspecialchars($event['event_name'] ?? '') ?></td>
            <td rowspan="<?= count($judges) ?>" style="white-space:nowrap;"><?= $event['event_date'] ? \App\Helpers\DateHelper::date($event['event_date']) : '—' ?></td>
            <td rowspan="<?= count($judges) ?>"><?= $sessBadge ?></td>
            <td rowspan="<?= count($judges) ?>" style="white-space:nowrap;"><?php
            ...
            ?></td>
            <td rowspan="<?= count($judges) ?>"><?= htmlspecialchars($event['venue_name'] ?? '—') ?></td>
            <?php endif; ?>
            <td>
                <strong><?= htmlspecialchars($judge['full_name'] ?? '') ?></strong>
                <br><span style="font-size:8pt;color:#6c757d;"><?= htmlspecialchars($judge['user_role'] ?? '') ?></span>
            </td>
            <td><?= htmlspecialchars($judge['department_name'] ?? '—') ?></td>
        </tr>
        <?php endforeach; endforeach; ?>
        <tr class="total-row">
            <td colspan="6" style="text-align:right;">Total</td>
            <td colspan="2"><?= $totalJudges ?> assignment(s) across <?= $totalEvents ?> event(s)</td>
        </tr>
        <?php endif; ?>
    </tbody>
</table>
...
<div class="footer">
    <div class="footer-content">
        ...
        <span>Judge Assignment Report &bull; <?= $symTitle ?></span>
