...
// Helper: prefer $old over $symposium for repopulation
function editValue(array $old, array $symposium, string $field): string
{
    if (isset($old[$field])) return (string) $old[$field];
    return (string) ($symposium[$field] ?? '');
}

$currentStatus = $symposium['status'] ?? '';
$isLocked      = in_array($currentStatus, [
    \App\Services\SymposiumService::STATUS_PENDING_HOD,
    \App\Services\SymposiumService::STATUS_PENDING_PRINCIPAL,
    \App\Services\SymposiumService::STATUS_APPROVED,
    \App\Services\SymposiumService::STATUS_REGISTRATION_OPEN,
    \App\Services\SymposiumService::STATUS_REGISTRATION_CLOSE,
    \App\Services\SymposiumService::STATUS_COMPLETED,
], true);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold mb-1"><i class="bi bi-pencil-square me-2 text-primary"></i>Edit Symposium</h2>
        <p class="text-muted mb-0">
            Code: <strong><?= htmlspecialchars((string) ($symposium['symposium_code'] ?? ''), ENT_QUOTES, 'UTF-8') ?></strong>
            &nbsp;|&nbsp;
            Status: <span class="badge bg-<?= symposium_status_badge_class($currentStatus) ?>"><?= htmlspecialchars($currentStatus, ENT_QUOTES, 'UTF-8') ?></span>
        </p>
    </div>
    <a href="<?= base_url() ?>/symposiums/view?id=<?= (int) ($symposium['symposium_id'] ?? 0) ?>" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left-circle me-1"></i> Back to View
    </a>
</div>
...
<?php if ($isLocked): ?>
    <div class="alert alert-warning d-flex gap-2 align-items-start">
        ...
        <div>
            <strong>Workflow Impact</strong> — This symposium is currently in the approval pipeline.
            Saving changes will reset the status to <strong>Revision Required</strong> and the approval workflow will restart from the beginning.
        </div>
    </div>
<?php endif; ?>

<form method="post" action="<?= base_url() ?>/symposiums/update"
    ...
    <div class="card shadow-sm border-0 mb-4">
        ...
        <div class="card-body">
            <div class="row g-4">
                ...
                <!-- Code (read-only) -->
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Symposium Code</label>
                    <input
                        type="text"
                ...
                <div class="col-md-8">
                    ...
                    <input
                        ...
                        value="<?= htmlspecialchars(editValue($old, $symposium, 'title'), ENT_QUOTES, 'UTF-8') ?>"
                        maxlength="150">
                    ...
                    <select name="academic_year" id="field-academic_year"
                        ...
                        <?php foreach ($academicYears as $year): ?>
                            <option
                                ...
                                <?= (editValue($old, $symposium, 'academic_year') === (string) $year) ? 'selected' : '' ?>>
                    ...
                    <select name="symposium_type" id="field-symposium_type"
                        ...
                        <?php foreach ($symposiumTypes as $type): ?>
                            <option
                                ...
                                <?= (editValue($old, $symposium, 'symposium_type') === $type) ? 'selected' : '' ?>>
                ...
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Organizing Departments <span class="text-danger">*</span></label>
                    <select
                        ...
                        <?php foreach ($departments as $dept): ?>
                            <option
                                ...
                                <?= in_array((int) $dept['department_id'], $selectedDeptIds, true) ? 'selected' : '' ?>>
                    ...
                    <div class="form-text">Hold Ctrl/Cmd for multiple.</div>
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea
                        ...
                        rows="4"><?= htmlspecialchars(editValue($old, $symposium, 'description'), ENT_QUOTES, 'UTF-8') ?></textarea>
        ...
        <div class="card-body">
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="datetime-local" name="registration_start" id="field-registration_start"
                        class="form-control <?= isset($errors['registration_start']) ? 'is-invalid' : '' ?>"
                        value="<?= htmlspecialchars(str_replace(' ', 'T', editValue($old, $symposium, 'registration_start')), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="datetime-local" name="registration_end" id="field-registration_end"
                        class="form-control <?= isset($errors['registration_end']) ? 'is-invalid' : '' ?>"
                        value="<?= htmlspecialchars(str_replace(' ', 'T', editValue($old, $symposium, 'registration_end')), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="event_start_date" id="field-event_start_date"
                        class="form-control <?= isset($errors['event_start_date']) ? 'is-invalid' : '' ?>"
                        value="<?= htmlspecialchars(editValue($old, $symposium, 'event_start_date'), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="event_end_date" id="field-event_end_date"
                        class="form-control <?= isset($errors['event_end_date']) ? 'is-invalid' : '' ?>"
                        value="<?= htmlspecialchars(editValue($old, $symposium, 'event_end_date'), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="d-flex gap-3 justify-content-end align-items-center">
        <a href="<?= base_url() ?>/symposiums/view?id=<?= (int) ($symposium['symposium_id'] ?? 0) ?>" class="btn btn-outline-secondary">Cancel</a>
        <button type="submit" class="btn btn-primary px-5" id="symSubmitBtn">
            <i class="bi bi-save me-1"></i>
            <?= $isLocked ? 'Save &amp; Reset Workflow' : 'Save Changes' ?>
        </button>
    </div>
