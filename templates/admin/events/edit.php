 ...
 * Description : Master Event Template Edit View.
 * -------------------------------------------------------------------------
 */
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="fw-bold mb-1">
            <i class="bi bi-pencil-square text-primary me-2"></i>Edit Master Event Template
        </h2>
    ...
    <div>
        <a href="<?= base_url() ?>/admin/events/view?id=<?= $event['event_id'] ?>" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-eye me-1"></i> View Details
        </a>
        <a href="<?= base_url() ?>/admin/events" class="btn btn-outline-secondary btn-sm ms-1">
            <i class="bi bi-arrow-left me-1"></i> Back to Library
        </a>
    </div>
</div>

<form method="POST" action="<?= base_url() ?>/admin/events/edit?id=<?= $event['event_id'] ?>" id="masterEventEditForm">
    <div class="row g-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="d-flex gap-3 pt-1">
                                <?php foreach ($formData['categories'] ?? ['Technical', 'Non-Technical'] as $cat): ?>
                        ...
                        <div class="col-md-3">
                            ...
                            <select name="status" class="form-select">
                                <?php foreach ($statuses as $st): ?>
                                    <option value="<?= $st ?>" <?= ($event['status'] ?? 'Draft') === $st ? 'selected' : '' ?>><?= $st ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <?php $durType = $event['duration_type'] ?? 'minutes'; ?>
                        <div class="col-12">
                                        ...
                                        <div class="col-auto">
                                            <div class="input-group" style="width: 200px;">
                                                <input type="number" name="duration_minutes" id="duration_minutes_input"
                                                       ...
                                                       value="<?= htmlspecialchars((string)($event['duration_minutes'] ?? '')) ?>">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="input-group">
                                                <input type="number" name="duration_days" min="1" max="30"
                                                       ...
                                                       value="<?= htmlspecialchars((string)($event['duration_days'] ?? 1)) ?>">
                ...
                <div class="card-body">
                    <div class="form-check form-switch mb-3">
                        ...
                        <input class="form-check-input" type="checkbox" name="requires_prelims" id="requires_prelims" value="1"
                               <?= !empty($event['requires_prelims']) ? 'checked' : '' ?>>
            ...
            <div class="card border-0 shadow-sm bg-light">
                <div class="card-body">
                    <button type="submit" class="btn btn-primary w-100 mb-2 py-2 fw-bold">
                        <i class="bi bi-save me-1"></i> Update Master Event
                    </button>
                    <a href="<?= base_url() ?>/admin/events/view?id=<?= $event['event_id'] ?>" class="btn btn-outline-secondary w-100 btn-sm">Cancel</a>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 3. WYSIWYG Rules Editor — sync contenteditable -> hidden textarea on form submit
    ...
    const editEventForm = document.querySelector('form[method="POST"]');

    // Sync on form submit
    if (editEventForm && wysiwygEditor && hiddenTextarea) {
